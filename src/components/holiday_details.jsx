...
// http://127.0.0.1:3232/holidays/package/details/650e44c8b87e434d3ad7ed6e
...
const HolidayDetails = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`http://127.0.0.1:3232/holidays/package/details/${id}`, { credentials: 'include' });
    ...
    return (
      ...
      <div className="holidayDetails">
        <Container maxWidth="xl">
          <HolidaysDetails PackageName={apidata.packageName} duration={apidata.packageDuration} changePrice={priceUpdate} price={apidata.price} />

          <div className="hd-level">
            <h4 className="itinerary" onClick={() => setActiveComponent('itinerary')}>ITINERARY</h4>
            <h4 className="policies" onClick={() => setActiveComponent('policies')}>POLICIES</h4>
            <h4 className="summary" onClick={() => setActiveComponent('summary')}>SUMMARY</h4>
          </div>

          <div className="hd-body">
            <div className="hd-main-section">
              {renderComponent()}
            </div>
            <div className="price-calc">
              ...
              <div className="net-price">
                <p><span>₹{apidata.price}</span> per person*</p>

                <p>{carprice && <h3 style={{ color: 'rgba(30, 33, 37, 1)', padding: '0px 10px', fontSize: '12px' }}> ₹ {carprice} <span style={{ fontSize: '10px', color: 'rgba(112, 117, 124, 1)' }}>Vehicle upgraded</span></h3 >}</p>
                <p>{hotelprice && <h3 style={{ color: 'rgba(30, 33, 37, 1)', padding: '0px 10px', fontSize: '12px' }}> ₹ {hotelprice} <span style={{ fontSize: '10px', color: 'rgba(112, 117, 124, 1)', }}>Hotel upgraded</span></h3 >}</p>
                <p>{packageprice && <h3 style={{ color: 'rgba(30, 33, 37, 1)', padding: '0px 10px', fontSize: '12px' }}> ₹ {packageprice} <span style={{ fontSize: '10px', color: 'rgba(112, 117, 124, 1)', }}>Total package</span></h3 >}</p>

                <p>*Excluding applicable taxes</p>
              </div>
              <div style={{ display: 'flex', flexDirection: 'row-reverse' }}>
                <p>{price && <h3 style={{ color: 'rgba(30, 33, 37, 1)', fontSize: '12px' }}>Total<span style={{fontSize: '1.5rem', fontWeight: 500,color:'red'}}> ₹{carprice + hotelprice + packageprice}</span></h3>}</p>
              </div>

              <h4 className="booking-process">
                Book Now
              </h4>
            </div>
          </div>
...
//                 return (
//                   <ActCard url={`http://127.0.0.1:3232/activity?id=${act.activityID}&type=${act.objType}`} type="hotel" id = {id}/>
...
//               if (act.objType ==='car'){
//                 return (
//                   <ActCard url={`http://127.0.0.1:3232/activity?id=${act.activityID}&type=${act.objType}`} type="car" id = {id}/>

//               )
//               }
//               if (act.objType ==='activity'){
//                   return (
//                     <ActCard url={`http://127.0.0.1:3232/activity?id=${act.activityID}&type=${act.objType}`} type="activity" />
