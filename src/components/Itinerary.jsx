   ...
   let id = apidata.apidata._id
    useEffect(() =>{
        const fetchData = async () => {
            try {
              const response = await fetch(`http://127.0.0.1:3232/holidays/package/iti/${id}`,{credentials:'include'});
    ...
    <div className="days-activity">
        <div className="days-title">
              <div className="days-num">
                <h4>Day {iti.dayCount} - {wordToTitleCase(iti.place)}</h4>
              </div>
            ...
            <span>{divideSentence(wordToTitleCase(iti.title))[0]} </span> {divideSentence(wordToTitleCase(iti.title))[1]}
            </h5>
            <div className="hd-activities">
            {iti.activities.map((act) =>{
            if (act.objType ==='car'){
                  return (
                    <ActCard url={`http://127.0.0.1:3232/activity?id=${act.activityID}&type=${act.objType}`} type="car" id = {id} changeprice = {apidata.changePrice}/>
        
                )
            }
            if (act.objType ==='hotel'){
                return (
                  <ActCard url={`http://127.0.0.1:3232/activity?id=${act.activityID}&type=${act.objType}`} type="hotel" id = {id} changeprice = {apidata.changePrice}/>
