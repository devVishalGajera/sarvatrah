...
useEffect(() =>{
  const fetchData = async () => {
      try {
        const response = await fetch(`http://127.0.0.1:3232/holidays/package/iti/hotel/update/${id.id}`,{credentials:'include'});
        ...
        <List>
          ...
          return(
               ...
               >
                 ...
                 >
                  {data.rooms.map((room) =>( <MenuItem value={room.roomType}>{room.roomType}</MenuItem>))}
                 ...
                 <ListItemSecondaryAction>
                   <IconButton edge="end" aria-label="add" onClick={() => handleAddToCart(payableFunc())}>
